...
const WECOM_WEBHOOK_KEY = process.env.WECOM_WEBHOOK_KEY;
const WECOM_WEBHOOK_BASE_URL = 'https://qyapi.weixin.qq.com/cgi-bin/webhook/send';

async function sendWeComMessage(message, imagePath = null) {
    if (!WECOM_WEBHOOK_KEY) {
        console.warn('[WeCom] Webhook key not configured. Skipping notification.');
        return false;
    }
    
    const WECOM_WEBHOOK_URL = `${WECOM_WEBHOOK_BASE_URL}?key=${WECOM_WEBHOOK_KEY}`;
    ...
    try {
        // 移除Markdown格式，转为纯文本
        let plainTextMessage = message.replace(/\*\*/g, "")
                                    .replace(/\*/g, "");
        
        // 检查消息长度，企业微信限制2048字节
        if (Buffer.byteLength(plainTextMessage, 'utf8') > 2048) {
            plainTextMessage = plainTextMessage.substring(0, 2045) + '...';
            console.warn('[WeCom] Message too long, truncated.');
        }
        ...
        const response = await axios.post(WECOM_WEBHOOK_URL, data, {
            timeout: 10000, // 10秒超时
            headers: {
        ...
        if (response.status === 200 && response.data.errcode === 0) {
            ...
            console.error('[WeCom] Failed to send message:', response.data);
        }
    } catch (e) {
        console.error('[WeCom] Failed to send message:', e.message);
        if (e.response) {
            console.error('[WeCom] Response data:', e.response.data);
            console.error('[WeCom] Response status:', e.response.status);
        }
    }

    // 2. 发送图片 (如果有)
    if (imagePath && fs.existsSync(imagePath)) {
        console.log('[WeCom] Sending photo...');
        try {
            // 读取图片文件
            ...
            if (imageBuffer.length > 2 * 1024 * 1024) {
                console.error('[WeCom] Photo too large (max 2MB). Skipping.');
                return messageSent;
            }
            ...
            const data = {
                ...
                image: {
                    base64: base64Image,
                    md5: require('crypto').createHash('md5').update(imageBuffer).digest('hex')
                }
            };
            
            const response = await axios.post(WECOM_WEBHOOK_URL, data, {
                timeout: 30000, // 图片发送超时时间更长
                headers: {
                    'Content-Type': 'application/json'
                }
            });
            ...
            if (response.status === 200 && response.data.errcode === 0) {
                console.log('[WeCom] Photo sent successfully.');
                photoSent = true;
            } else {
                console.error('[WeCom] Failed to send photo:', response.data);
            }
        } catch (e) {
            console.error('[WeCom] Failed to send photo:', e.message);
            if (e.response) {
                console.error('[WeCom] Response data:', e.response.data);
                console.error('[WeCom] Response status:', e.response.status);
            }
            ...
            for (let attempt = 1; attempt <= 20; attempt++) {
                ...
                if (await renewBtn.isVisible()) {
                    ...
                    for (let findAttempt = 0; findAttempt < 15; findAttempt++) {
                        cdpClickResult = await attemptTurnstileCdp(page);
                        if (cdpClickResult) break;
                        console.log(`   >> [寻找尝试 ${findAttempt + 1}/15] 尚未找到 Turnstile 复选框...`);
                        await page.waitForTimeout(1000);
                    }
